"""Train the defender deployment policy with PPO.

Each RL step commits a placement, runs the episode, and returns the
episode's defender reward.
"""
...
from envs import DeploymentEnv, EnvConfig
from training.common import EpisodeStatsCallback, load_config, make_vec_env


def _tactical_fn_from_model(model_path: str):
...
def make_env_fn(cfg: EnvConfig, seed: int, tactical_model: str | None):
    def _thunk():
        tac_fn = _tactical_fn_from_model(tactical_model) if tactical_model else None
        env = DeploymentEnv(cfg=cfg, seed=seed, tactical_fn=tac_fn)
        env = Monitor(env)
        return env
    return _thunk
...
def main():
    ...
    parser.add_argument("--out", default="outputs/models/deployment")
    parser.add_argument("--log", default="outputs/plots/deployment_training.csv")
    ...
    lr = conf["training"]["deployment"]["lr"]

    vec_env = make_vec_env(
        lambda i: make_env_fn(env_cfg, seed=3000 + i,
